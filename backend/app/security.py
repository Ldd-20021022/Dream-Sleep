...
from app.redis_client import redis_rate_check as _redis_rate, redis_blacklist_add as _redis_bl_add, redis_blacklist_check as _redis_bl_check, redis_cache_get, redis_cache_set, redis_cache_delete
...
def rate_limit(key: str, max_requests: int = 30, window: int = 60) -> bool:
    """Returns True if request is allowed, False if rate limited. Uses Redis in production."""
    try:
        return _redis_rate(f"rate:{key}", max_requests, window)
    except Exception:
...
def blacklist_token(token: str, ttl: int = 604800):  # 7 days
    """Add JWT to blacklist (on logout). Redis + in-memory fallback."""
    token_hash = hashlib.sha256(token.encode()).hexdigest()[:32]
    jwt_blacklist.add(token_hash)
    cache.set(f"bl:{token_hash}", True, ttl)
    try:
        _redis_bl_add(token_hash, ttl)
    except Exception:
        pass
