...
# Safe table creation: skip if tables already exist (avoids worker race)
try:
    Base.metadata.create_all(bind=engine)
except Exception:
    pass  # Tables already exist from another worker

# CORS — tightened for production
if _settings.PRODUCTION:
    _origins = os.getenv("ALLOWED_ORIGINS", "https://p1.ansycare.com").split(",")
else:
    _origins = ["http://localhost:8000", "http://127.0.0.1:8000", "http://localhost:3000", "http://localhost:5173"]
app.add_middleware(
    CORSMiddleware,
